...
class FirstReadyFCFSModel(cfg: FirstReadyFCFSConfig)(implicit p: Parameters)
  ...
  val powerStats = (rankStateTrackers).zip(UIntToOH(cmdRank).asBools).map { case (rankState, cmdUsesThisRank) =>
