...
import Console.{GREEN, RESET, UNDERLINED}
...
abstract class BaseDRAMMMRegIO(cfg: DRAMBaseConfig) extends MMRegIO(cfg) with HasConsoleUtils {
  ...
  val dramBaseRegisters = Seq(
    ...
    (backendLatency -> RuntimeSetting(2, "Backend Latency", min = 1, max = Some(1 << (cfg.backendKey.latencyBits - 1)))),
  )
...
class FirstReadyFCFSEntry(nastiParams: NastiParameters, key: DRAMBaseConfig)(implicit p: Parameters)
    extends MASEntry(nastiParams, key)(p) {
...
class BankStateTrackerO(val key: DramOrganizationParams) extends Bundle with CommandLegalBools {
...
class BankStateTrackerIO(val key: DramOrganizationParams) extends Bundle with HasLegalityUpdateIO {
...
class RankStateTrackerO(val key: DramOrganizationParams) extends Bundle with CommandLegalBools {
  import DRAMMasEnums._
...
class RankStateTrackerIO(val key: DramOrganizationParams)
    extends Bundle
...
class RankStateTracker(key: DramOrganizationParams) extends Module with HasDRAMMASConstants {
  ...
  when(io.cmdUsesThisRank && io.selectedCmd === cmd_act) {
    ...
    assert(!io.cmdUsesThisRank || io.rank.canCASR, "Rank Timing Violation: Controller issued CASR command illegally")
    ...
    assert(!io.cmdUsesThisRank || io.rank.canCASW, "Rank Timing Violation: Controller issued CASW command illegally")
    nextLegalCASR.io.set.valid := true.B
    nextLegalCASR.io.set.bits  := Mux(
      io.cmdUsesThisRank,
...
class RefreshUnit(key: DramOrganizationParams) extends Module {
  ...
  val prechargeableRanks = VecInit(io.rankStati.map { rank =>
    rank.canPRE &&
    (rank.banks.map { _.canPRE }.reduce { _ || _ })
  }).asUInt & io.ranksInUse
